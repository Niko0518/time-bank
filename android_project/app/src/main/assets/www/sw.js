// Time Bank Service Worker - v9.17.10
...
const CACHE_NAME = 'timebank-cache-v9.17.10';
const ASSETS = [
    './',
    './index.html',
    './manifest.json',
...
// [v9.10.0] PWA Background Sync：后台网络恢复时自动触发同步
        ...
        event.waitUntil(
            self.clients.matchAll().then(clients => {
                clients.forEach(client => {
                    client.postMessage({ type: 'SW_SYNC_MUTATIONS' });
                });
            })
        );
    }
...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'REGISTER_SYNC') {
        console.log('[SW] Received REGISTER_SYNC from client');
        event.waitUntil(
            self.registration.sync.register('sync-mutations').catch(err => {
                // SyncManager 不支持时静默失败（iOS Safari 等）
                console.warn('[SW] sync.register failed:', err.message);
            })
        );
    }
});
