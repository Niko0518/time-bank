...
// [v9.18.0] 迷你卡片计时格式：仅保留一个时间单位
function formatTimeMini(seconds) { if (seconds === null || isNaN(seconds) || seconds < 0) return '0秒'; seconds = Math.round(seconds); if (seconds < 60) return `${seconds}秒`; if (seconds < 3600) return `${Math.floor(seconds / 60)}分`; return `${Math.floor(seconds / 3600)}时`; }
