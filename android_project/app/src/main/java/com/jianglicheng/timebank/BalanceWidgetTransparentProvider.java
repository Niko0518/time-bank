...
/**
 * [v7.14.0] 时间余额桌面小组件 - 通透模式方案三：高透明渐变
 * 根据余额区间切换高透明度渐变背景
 */
public class BalanceWidgetTransparentProvider extends AppWidgetProvider {

    public static final String ACTION_UPDATE = "com.jianglicheng.timebank.WIDGET_TRANSPARENT_UPDATE";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_UPDATE.equals(intent.getAction())) {
            ...
            int[] appWidgetIds = appWidgetManager.getAppWidgetIds(
                    new ComponentName(context, BalanceWidgetTransparentProvider.class));
            for (int appWidgetId : appWidgetIds) {
                updateAppWidget(context, appWidgetManager, appWidgetId);
            }
        }
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_balance_transparent);
        
        // [v7.14.0] 生成渐变余额数字 Bitmap
        Bitmap gradientBalanceBitmap = createGradientTextBitmap(context, formattedBalance, balanceSeconds, 28);
        views.setImageViewBitmap(R.id.widget_balance_transparent_amount, gradientBalanceBitmap);
        
        views.setInt(R.id.widget_balance_transparent_container, "setBackgroundResource", bgDrawable);
        
        Intent intent = new Intent(context, MainActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        views.setOnClickPendingIntent(R.id.widget_balance_transparent_container, pendingIntent);
