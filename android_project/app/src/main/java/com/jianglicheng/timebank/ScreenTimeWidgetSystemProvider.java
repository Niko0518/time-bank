...
/**
 * [v7.14.0] 屏幕时间桌面小组件 - 通透模式方案二：系统透明
 * 使用系统透明背景，依赖系统 launcher 的模糊处理
 */
public class ScreenTimeWidgetSystemProvider extends AppWidgetProvider {

    public static final String ACTION_UPDATE = "com.jianglicheng.timebank.SCREEN_TIME_SYSTEM_UPDATE";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_UPDATE.equals(intent.getAction())) {
            ...
            int[] appWidgetIds = appWidgetManager.getAppWidgetIds(
                    new ComponentName(context, ScreenTimeWidgetSystemProvider.class));
            for (int appWidgetId : appWidgetIds) {
                updateAppWidget(context, appWidgetManager, appWidgetId);
            }
        }
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_screen_time_system);
        
        // [v7.14.0] 生成渐变大百分比文字 Bitmap
        Bitmap gradientPercentBitmap = createGradientTextBitmap(context, percentText, percent, 20);
        views.setImageViewBitmap(R.id.widget_st_system_percent, gradientPercentBitmap);
        ...
        views.setImageViewBitmap(R.id.widget_st_system_used_limit, gradientUsedLimitBitmap);
        
        Intent intent = new Intent(context, MainActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, appWidgetId, intent,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        views.setOnClickPendingIntent(R.id.widget_st_system_container, pendingIntent);
