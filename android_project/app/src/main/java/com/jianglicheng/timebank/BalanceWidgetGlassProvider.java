...
/**
 * [v7.14.0] 时间余额桌面小组件 - 通透模式方案一：毛玻璃
 * 使用半透明背景模拟毛玻璃效果
 */
public class BalanceWidgetGlassProvider extends AppWidgetProvider {

    public static final String ACTION_UPDATE = "com.jianglicheng.timebank.WIDGET_GLASS_UPDATE";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_UPDATE.equals(intent.getAction())) {
            ...
            int[] appWidgetIds = appWidgetManager.getAppWidgetIds(
                    new ComponentName(context, BalanceWidgetGlassProvider.class));
            for (int appWidgetId : appWidgetIds) {
                updateAppWidget(context, appWidgetManager, appWidgetId);
            }
        }
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_balance_glass);
        
        // [v7.14.0] 生成渐变余额数字 Bitmap
        Bitmap gradientBalanceBitmap = createGradientTextBitmap(context, formattedBalance, balanceSeconds, 28);
        views.setImageViewBitmap(R.id.widget_balance_glass_amount, gradientBalanceBitmap);
        
        Intent intent = new Intent(context, MainActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        views.setOnClickPendingIntent(R.id.widget_balance_glass_container, pendingIntent);
