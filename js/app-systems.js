...
function updateStackedContainerVisibility(skipLayoutUpdate = false) {
    ...
    if (stackedContainer) {
        ...
        // [v9.30.1] 修正：屏幕时间不可见但睡眠卡片可见时，睡眠卡片作为堆叠第一张卡片
        //           仍需保持容器负 margin 形成堆叠效果；仅当两者都不可见时才恢复间距
        stackedContainer.classList.toggle('st-no-screen-time', !screenTimeVisible && !sleepVisible);
    }
...
function autoSettleScreenTime() {
    ...
    if (!hasCompletedFirstCloudSync && isLoggedIn()) {
        console.warn('[ScreenTime] 云端数据未就绪，跳过结算');
        return emptyResult;
    }
