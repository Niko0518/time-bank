...
async function tryAutoReLogin() {
    ...
    try {
        ...
        // [v9.4.0] 登录成功后启动同步监控 + 拉一次 MQTT 配置
        if (typeof window.__startSyncMonitor === 'function') {
            window.__startSyncMonitor();
        }
        // 存 openid 到 localStorage 供 __refreshMqttConfig 用
        try {
            if (cachedLoginState && cachedLoginState.userInfo) {
                localStorage.setItem('tb_openid', cachedLoginState.userInfo.openid || cachedLoginState.userInfo.uid || '');
            }
        } catch (_) {}
...
// [v9.5.0] 自愈探针已删除：移除 __watchSelfHealingCountdown / __watchSelfHealingTimer / __watchSelfHealingProbeCount / __watchCountdownTicker
...
// [v9.5.0] 移除 WATCH_SELF_HEAL_INTERVAL_MS（自愈探针已删除）
...
// [v9.5.0] 移除自愈探针相关字段（probeCountdown / probeCount）
...
// [v9.5.0] 移除自愈探针相关字段（__watchSelfHealingCountdown / __watchSelfHealingProbeCount / __stopWatchSelfHealingProbe / __stopSelfHealingCountdownTicker）
...
// [v9.5.0] 自愈探针已删除 —— 由"全屏降级警告横幅"替代
// ----------------------------------------------------------------------------
// 历史：v9.0.10 引入 __startWatchSelfHealingProbe，期望"降到 paused 状态时后台 60s 自动探活"。
// 实际效果：探活几乎从未成功恢复连接（WebSocket 重建依赖 SDK 状态、用户认证、网络层多重因素，
//          60s 一次的间隔太短；触发的 toast 反而干扰用户）。
// v9.5.0 重构：完全删除自愈探针。降级状态下不再有"自动恢复"幻觉，改为"明确告知用户 + 一键重试"。
// 新机制：state:paused 事件触发全屏降级警告横幅（用户必须看到），横幅内提供"立即重试"按钮
//         和"诊断详情"入口，由用户主动决策。
// ----------------------------------------------------------------------------
...
function startWatchHeartbeatWatchdog() {
    ...
    function check() {
        ...
        if (staleWatchers.length > 0) {
            // [v9.0.11-fix] 限频：清理 1h 窗口外的时间戳，超限进入降级状态
            // [v9.5.0] 自愈探针已删除：进入 paused 状态后只启动轻量 watchdog 探针（与原自愈探针独立）
            ...
            if (__watchdogActionTimestamps.length >= MAX_WATCHDOG_ACTIONS_PER_HOUR) {
                console.warn(`🐕 [Watchdog] 1h 内已重建 ${__watchdogActionTimestamps.length} 次，暂停自动重建，进入降级状态`);
                __watchDegradeStatus = 'paused';
                __recordWatchDegrade();
                // [v9.5.0] 启动 60s 一次的轻量 watchdog 探针（与已删除的 __startWatchSelfHealingProbe 独立）
                ...
                // [v9.5.0] 触发降级警告横幅（用户必须看到）
                if (typeof showWatchDegradedWarning === 'function') {
                    showWatchDegradedWarning();
                }
                updateWatchStatusUI();
                watchHeartbeatTimer = setTimeout(check, WATCH_HEARTBEAT_CHECK_INTERVAL);
                return;
...
// [v9.5.0] Watch 断线自动重连调度器 —— 兼容层（实际逻辑已迁移到 WatchReconnectScheduler）
// ----------------------------------------------------------------------------
// v9.5.0 重构：原 110+ 行单一函数（混合 5 个职责）已拆分为 4 层架构：
//   - WatchEventBus（事件总线）
//   - ReconnectStrategy（策略模式：指数退避算法）
//   - WatchStateManager（状态机 + 持久化）
//   - WatchReconnectExecutor（业务执行）
//   - WatchReconnectScheduler（调度器）
// 实际实现见 app-1.js 末尾追加的 IIFE 模块（约 8700-9210 行）。
// 此函数保留仅为保持 5 处 onError 回调的调用兼容性（task/transaction/running/profile/daily）。
function scheduleWatchReconnect(reason = 'error') {
    if (typeof __watchReconnectScheduler !== 'undefined' && __watchReconnectScheduler) {
        __watchReconnectScheduler.schedule(reason);
    } else {
        // 极端情况：新架构尚未加载（如脚本加载顺序异常），降级到最小重试
        console.warn('[Watch] 新调度器未就绪，降级到 setTimeout（10s）');
        setTimeout(() => {
            try { DAL.subscribeAll(); } catch (e) { console.error('[Watch] 降级重连失败:', e); }
        }, 10000);
    }
}
...
// [v9.0.10 完善] 启动时恢复 Watch 降级状态（跨刷新持续）
// [v9.5.0] 自愈探针已删除：不再在启动时启动探针，改为"降级警告横幅 + 用户主动重试"
// 必须在 initApp 早期调用
function __initWatchDegradeState() {
    try {
        __loadWatchDegradeState();
        // [v9.5.0] 如果上次是 paused 状态，触发降级警告横幅（用户必须看到）
        if (__watchDegradeStatus === 'paused' && typeof isLoggedIn === 'function' && isLoggedIn()) {
            console.log('⚠️ [Watch] 检测到上次降级状态，显示警告横幅等待用户操作');
            if (typeof showWatchDegradedWarning === 'function') {
                showWatchDegradedWarning();
            }
        }
...
// [v9.5.0] 自愈探针字段已移除：不再显示"自愈倒计时"和"探针次数"
function showWatchDiagnostics() {
    try {
        ...
        // [v9.2.3] 启动自动刷新：每 1s 刷新状态文字（避免显示"死数字"）
...
// [v9.2.3] 诊断面板自动刷新：每 1s 更新状态文字
// [v9.5.0] 自愈探针字段已移除：不再刷新倒计时
...
// [v9.5.0] 监听状态显示器 —— 重构版
// ----------------------------------------------------------------------------
// 设计原则（用户原话："重构监听状态显示器，更加美观、简洁、直观"）：
//   1. 状态分 5 级，每级有专属颜色 + emoji + 1-2 字短文
//   2. 降级状态（paused）使用强视觉提示：背景脉冲 + 警示色 + "需手动重试"
//   3. 倒计时内嵌：保活中显示"X 秒后重试"，让用户感知系统在努力
//   4. hover 提示详细但不啰嗦：1 行状态 + 1 行说明 + 1 行操作建议
//   5. 防抖：100ms 内多次状态变化合并为一次 DOM 更新
//   6. 事件驱动：监听 __watchEventBus 自动响应 state:changed
// ----------------------------------------------------------------------------

// 状态视觉规范（5 级）
const WATCH_STATUS_VISUAL = {
    ok:        { icon: '🟢', label: '已同步', class: 'watch-active' },
    connecting:{ icon: '🟡', label: '连接中', class: 'watch-connecting' },
    degraded:  { icon: '🟠', label: '保活中', class: 'watch-connecting' },
    paused:    { icon: '🔴', label: '已暂停', class: 'watch-inactive' },
    offline:   { icon: '⚫', label: '未登录', class: 'watch-inactive' }
};

function updateWatchStatusUI() {
    // [v9.2.3] 防抖：100ms 内多次调用合并
...
function __updateWatchStatusUIInternal() {
    ...
    const degradeStatus = (typeof __watchDegradeStatus !== 'undefined') ? __watchDegradeStatus : 'ok';

    // === 状态判定（优先级：paused > degraded > connecting > ok > offline）===
    let visual;
    let statusText = '';
    let tooltip = '';

    if (!userLoggedIn) {
        visual = WATCH_STATUS_VISUAL.offline;
        const reasonText = (!cachedLoginState && cloudbaseInitialized && auth?.hasLoginState?.())
            ? '登录态恢复中...' : '请登录后启用云端同步';
        statusText = visual.label;
        tooltip = `⚫ 未登录\n${reasonText}\n点击查看详情`;
    } else if (degradeStatus === 'paused') {
        // 🔴 降级暂停（用户最关心的状态）
        visual = WATCH_STATUS_VISUAL.paused;
        const elapsed = __watchFirstFailAt ? Math.floor((Date.now() - __watchFirstFailAt) / 1000) : 0;
        const failCount = (typeof __watchFailCount !== 'undefined') ? __watchFailCount : 0;
        const reason = __watchLastReason || '未知';
        const failCountText = failCount > 0 ? ` (失败 ${failCount} 次)` : '';
        statusText = `${visual.label} ${elapsed}s${failCountText}`;
        tooltip = `🔴 Watch 自动重连已停止\n原因：${reason}\n操作：点击 → 诊断 / 手动重试`;
    } else if (degradeStatus === 'degraded') {
        // 🟠 保活中（偶发断开）
        visual = WATCH_STATUS_VISUAL.degraded;
        const failCount = (typeof __watchFailCount !== 'undefined') ? __watchFailCount : 0;
        const max = 8;
        // 倒计时后缀
        let countdownSuffix = '';
        if (typeof __watchNextReconnectAt !== 'undefined' && __watchNextReconnectAt > Date.now()) {
            ...
            countdownSuffix = ` ${sec}s`;
        }
        statusText = `${visual.label} ${failCount}/${max}${countdownSuffix}`;
        tooltip = `🟠 云端连接偶发断开\n${failCount}/${max} 次后转为暂停\n操作：无需干预，自动恢复中`;
        ...
        // 连接中 / 同步中
        visual = WATCH_STATUS_VISUAL.connecting;
        statusText = `${visual.label} ${connectedCount}/${totalWatchers}`;
        tooltip = `🟡 正在连接云端 (${connectedCount}/${totalWatchers})\n操作：等待自动连接完成`;
    } else {
        // 🟢 已同步（最理想状态）
        visual = WATCH_STATUS_VISUAL.ok;
    ...
    [earnStatusEl, spendStatusEl].forEach(el => {
        if (!el) return;
        el.className = `watch-status ${visual.class}`;
        el.title = tooltip;
        el.setAttribute('data-status', degradeStatus);
        // 降级状态标记（CSS 可针对此属性设计更明显的视觉）
        if (degradeStatus === 'paused') {
            el.classList.add('watch-degraded-paused');
        } else {
            el.classList.remove('watch-degraded-paused');
        }
        // 更新文字
        const textEl = el.querySelector('.watch-status-text');
        if (textEl) textEl.textContent = statusText;
        // 更新图标（点）
        const dotEl = el.querySelector('.watch-status-dot');
        if (dotEl) {
            // 使用 CSS 变量控制颜色，无需直接改 style
            dotEl.setAttribute('data-status', degradeStatus);
        }
    });
}

// [v9.0.10 修复] updateCloudStatusUI 改为薄包装
...
// [v9.5.0] 降级警告横幅 —— 强制提醒（用户原话"对降级处理感到恐惧，担心用户会一直处于降级状态而不自知"）
// ----------------------------------------------------------------------------
// 触发条件：state.status === 'paused'
// 行为：显示全屏（实际是顶部固定）红色横幅，包含：
//   - 明显的警示图标和文案
//   - 降级时长（持续累加）
//   - 失败原因（来自 state.lastReason）
//   - "立即重试" 按钮 → 调用 __watchReconnectScheduler.forceReconnect()
//   - "查看详情" 链接 → 打开 showWatchDiagnostics
//   - "X 关闭" 按钮（仅本次会话隐藏，重启后会重新显示）
// 关闭横幅时调用 hideWatchDegradedWarning()。
// ----------------------------------------------------------------------------
let __watchDegradedWarningShown = false;
let __watchDegradedWarningInterval = null;

function showWatchDegradedWarning() {
    const banner = document.getElementById('watchDegradedWarning');
    if (!banner) {
        console.warn('[Watch] 降级警告横幅 DOM 元素未找到（#watchDegradedWarning）');
        return;
    }
    if (__watchDegradedWarningShown) {
        // 已显示，仅更新文案
        __updateDegradedWarningContent();
        return;
    }
    __watchDegradedWarningShown = true;
    banner.classList.add('show');
    __updateDegradedWarningContent();

    // 启动每秒刷新（降级时长累加）
    if (__watchDegradedWarningInterval) clearInterval(__watchDegradedWarningInterval);
    __watchDegradedWarningInterval = setInterval(() => {
        if (!banner.classList.contains('show')) {
            clearInterval(__watchDegradedWarningInterval);
            __watchDegradedWarningInterval = null;
            return;
        }
        __updateDegradedWarningContent();
    }, 1000);

    console.warn('⚠️ [Watch] 降级警告横幅已显示（用户必须看到）');
}

function __updateDegradedWarningContent() {
    const state = (typeof __watchStateManager !== 'undefined' && __watchStateManager)
        ? __watchStateManager.getState()
        : { status: (typeof __watchDegradeStatus !== 'undefined') ? __watchDegradeStatus : 'paused',
            firstFailAt: __watchFirstFailAt || 0,
            lastReason: __watchLastReason || '未知',
            failCount: __watchFailCount || 0 };

    const elapsed = state.firstFailAt ? Math.floor((Date.now() - state.firstFailAt) / 1000) : 0;
    const reason = state.lastReason || '未知';
    const failCount = state.failCount || 0;

    const setText = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    setText('degradedWarningReason', reason);
    setText('degradedWarningFails', failCount > 0 ? `（连续 ${failCount} 次失败）` : '');
    setText('degradedWarningElapsed', formatElapsedTime(elapsed));
}
...
function hideWatchDegradedWarning() {
    const banner = document.getElementById('watchDegradedWarning');
    if (banner) banner.classList.remove('show');
    __watchDegradedWarningShown = false;
    if (__watchDegradedWarningInterval) {
        clearInterval(__watchDegradedWarningInterval);
        __watchDegradedWarningInterval = null;
    }
    console.log('[Watch] 降级警告横幅已隐藏');
}

// 立即重试（横幅按钮调用）
function watchDegradedRetryNow() {
    console.log('[Watch] 用户点击"立即重试"');
    if (typeof __watchReconnectScheduler !== 'undefined' && __watchReconnectScheduler) {
        __watchReconnectScheduler.forceReconnect('user-retry-from-warning');
    } else if (typeof DAL !== 'undefined' && DAL && typeof DAL.subscribeAll === 'function') {
        DAL.subscribeAll().catch(e => console.error('[Watch] 手动重连失败:', e));
    }
}

// [v9.5.0] 监听事件总线：状态变化时自动显示/隐藏警告横幅
// 在 IIFE 初始化后注册（延迟到 window 加载完成）
if (typeof window !== 'undefined') {
    // 用 setTimeout 0 延迟执行，确保 IIFE 内的 __watchEventBus 已就绪
    setTimeout(() => {
        if (typeof __watchEventBus !== 'undefined' && __watchEventBus) {
            __watchEventBus.on('state:paused', (data) => {
                if ((typeof isLoggedIn === 'function') && isLoggedIn()) {
                    showWatchDegradedWarning();
                }
            });
            __watchEventBus.on('state:recovered', () => {
                hideWatchDegradedWarning();
            });
        }
    }, 0);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // [v9.4.0] DOM 加载完成立即启动同步状态监控条（不依赖登录）
    if (typeof window.__startSyncMonitor === 'function') {
        window.__startSyncMonitor();
    }
});
...
// ============================================================
// [v9.4.0] 同步状态监控条（顶部 🟢/🟡/🔴 状态条）
// ============================================================
//
// 职责：
//   1. 每 5s 轮询 Android 原生层（getLongConnectionState / getGetuiPushState / ...）
//   2. 渲染顶部状态条（点 + 文字 + 颜色）
//   3. 点击状态条展开详情（MQTT/个推 PUSH/上次同步/待注入/原生层）
//   4. 启动时把 Android 拿到的个推 clientId 上报到 tb_profile.devicePushMap
//      （让 CloudBase tbPushRelay 知道该往哪个 clientId 发推送）
//
// 设计：
//   - 即使未登录也启动（仅显示"未连接"），保证状态条不卡住
//   - clientId 上传要求已登录（避免 1003 错误）
//   - 所有调用 try/catch，单个失败不影响其它

let __syncMonitorTimer = null;
let __syncDetailVisible = false;
let __pushClientIdRegistered = false; // 本次会话已上报过
let __mqttConfigRefreshed = false; // 本次会话已写入 broker 配置
 ...
 * - 立即触发一次，避免 5s 空白
 */
window.__startSyncMonitor = function() {
    try {
        if (__syncMonitorTimer) return; // 幂等
        __renderSyncBar(); // 立即渲染一次（让"初始化中"消失）
        __syncMonitorTimer = setInterval(() => {
            __renderSyncBar();
        }, 5000);
    } catch (e) {
        console.error('[v9.4.0] __startSyncMonitor 失败:', e);
    }
};
 ...
 * [v9.4.0] 点击状态条 → 切换详情面板
 */
window.__toggleSyncDetail = function() {
    try {
        const detail = document.getElementById('tbSyncDetail');
        if (!detail) return;
        __syncDetailVisible = !__syncDetailVisible;
        detail.style.display = __syncDetailVisible ? 'block' : 'none';
        if (__syncDetailVisible) {
            // 展开时立即刷新一次详情内容
            __renderSyncDetail();
        }
    } catch (e) {
        console.error('[v9.4.0] __toggleSyncDetail 失败:', e);
    }
};

/**
 * [v9.4.0] 渲染状态条（主条 + 详情）
 * 优先用 Android 端 getNativeSyncState() 综合接口（一次返回 7 个字段），
 * 该方法在 WebAppInterface.java v9.4.0 段已实现。
 */
function __renderSyncBar() {
    try {
        const textEl = document.getElementById('tbSyncText');
        const dotEl = document.getElementById('tbSyncDot');
        if (!textEl || !dotEl) return;

        // 未挂载 Android 桥（开发期 PWA 调试）→ 兜底
        if (!window.Android || typeof window.Android.getNativeSyncState !== 'function') {
            textEl.textContent = '同步状态：浏览器模式（无原生桥）';
            dotEl.style.background = '#9E9E9E';
            dotEl.style.boxShadow = '0 0 4px #9E9E9E';
            return;
        }
        ...
        let state;
        try {
            const raw = window.Android.getNativeSyncState() || '{}';
            state = JSON.parse(raw);
        } catch (e) {
            state = { longConnState: 'DISCONNECTED', getuiClientId: null, nativeSyncActive: false };
        }

        // 2. 状态 → 文字 + 颜色
        const longConnState = state.longConnState || 'DISCONNECTED';
        const hasGetui = !!state.getuiClientId;
        const map = {
            'CONNECTED':    { text: '同步状态：已连接 🟢',  color: '#4CAF50' },
            'CONNECTING':   { text: '同步状态：连接中 🟡',  color: '#FFC107' },
            'RECONNECTING': { text: '同步状态：重连中 🟡',  color: '#FFC107' },
            'DISCONNECTED': { text: '同步状态：未连接 🔴',  color: '#F44336' }
        };
        const m = map[longConnState] || map['DISCONNECTED'];
        const getuiText = hasGetui ? '📱 PUSH✓' : '📱 PUSH✗';
        textEl.textContent = m.text + ' · ' + getuiText;
        dotEl.style.background = m.color;
        dotEl.style.boxShadow = '0 0 4px ' + m.color;

        // 3. 详情面板可见时刷新内容
        if (__syncDetailVisible) {
            __renderSyncDetail(state);
        }

        // 4. 顺便尝试上报 clientId（仅首次 + 已登录 + 已注册 clientId）
        if (!__pushClientIdRegistered && hasGetui && isLoggedIn()) {
            __registerPushClientIdOnce();
        }
        // 5. 顺便尝试把 broker 配置写入 SharedPreferences（仅首次；不要求已登录，函数内自兜底）
        if (!__mqttConfigRefreshed) {
            __refreshMqttConfig();
        }
    } catch (e) {
        console.error('[v9.4.0] __renderSyncBar 失败:', e);
    }
}

/**
 * [v9.4.0] 渲染详情面板 5 个字段
 * 接受可选 state 参数（已从 getNativeSyncState 解析），避免重复调用
 */
function __renderSyncDetail(state) {
    try {
        ...
        // 1. MQTT 长连接
        const longEl = $('tbSyncLongConn');
        if (longEl) {
            const s = state.longConnState || 'DISCONNECTED';
            const map = {
                'CONNECTED':    '🟢 已连接',
                'CONNECTING':   '🟡 连接中',
                'RECONNECTING': '🟡 重连中',
            ...
            longEl.textContent = map[s] || ('⚪ ' + s);
        }

        // 2. 个推 PUSH
        const getuiEl = $('tbSyncGetui');
        if (getuiEl) {
            if (state.getuiClientId) {
                getuiEl.textContent = '📱 已注册 (' + String(state.getuiClientId).slice(0, 8) + '...)';
            } else {
                getuiEl.textContent = '⚪ 未注册';
            }
        }

        // 3. 上次同步
        const lastEl = $('tbSyncLastAt');
        if (lastEl) {
            const ts = state.lastDeltaSyncAt || 0;
            if (!ts || ts === 0) {
                lastEl.textContent = '⚪ 从未同步';
            } else {
                const d = new Date(ts);
                lastEl.textContent = '🕐 ' + d.toLocaleTimeString('zh-CN', { hour12: false });
            }
        }

        // 4. 待注入（pendingDeltaCount + getuiPushReceived 都显示）
        const pendingEl = $('tbSyncPending');
        if (pendingEl) {
            const pending = state.pendingDeltaCount || 0;
            const pushed = state.getuiPushReceived || 0;
            pendingEl.textContent = '📨 差集 ' + pending + ' · PUSH ' + pushed;
        }

        // 5. 原生层活跃
        const nativeEl = $('tbSyncNativeActive');
        if (nativeEl) {
            nativeEl.textContent = state.nativeSyncActive ? '🟢 服务在跑' : '⚪ 未启动';
        }
    } catch (e) {
        console.error('[v9.4.0] __renderSyncDetail 失败:', e);
    }
}

/**
 * [v9.4.0] 把 Android 拿到的个推 clientId + 本机 deviceId 上报到 tb_profile.devicePushMap
 * tbPushRelay 据此向该 clientId 推透传消息
 * 幂等：本次会话只上报一次（__pushClientIdRegistered 守护）
 * 失败兜底：5 分钟后重试（__syncMonitorTimer 5s 间隔会再触发）
 */
async function __registerPushClientIdOnce() {
    try {
        if (!window.Android) return;
        if (!isLoggedIn()) return;
        const clientId = (window.Android.getGetuiClientId && window.Android.getGetuiClientId()) || '';
        const deviceId = (window.Android.getTbDeviceId && window.Android.getTbDeviceId()) || '';
        if (!clientId || !deviceId) {
            console.log('[v9.4.0] registerPushClientId 跳过：clientId 或 deviceId 为空');
            return;
        }
        if (!app) {
            console.warn('[v9.4.0] registerPushClientId 跳过：app 未初始化');
            return;
        }
        // 拿 currentUid（DAL.getCurrentUid 是 DAL 类方法，不是顶层函数）
        // 兜底：直接从 cachedLoginState.user.uid 取，避免依赖 DAL 加载时机
        let currentUid = null;
        try {
            if (typeof DAL !== 'undefined' && DAL && typeof DAL.getCurrentUid === 'function') {
                currentUid = await DAL.getCurrentUid();
            }
        } catch (_) {}
        if (!currentUid && typeof cachedLoginState !== 'undefined' && cachedLoginState && cachedLoginState.user) {
            currentUid = cachedLoginState.user.uid || cachedLoginState.user.openid || cachedLoginState.user.id || null;
        }
        if (!currentUid) {
            console.warn('[v9.4.0] registerPushClientId 跳过：未拿到 currentUid (DAL.getCurrentUid=' + (typeof DAL !== 'undefined' && typeof DAL.getCurrentUid) + ')');
            return;
        }
        console.log('[v9.4.0] 上报 PUSH clientId device=' + deviceId.slice(0, 8) + '... clientId=' + clientId.slice(0, 8) + '...');
        const result = await app.callFunction({
            name: 'tbMutation',
            data: {
                action: 'registerPushClientId',
                _openid: currentUid,
                data: { deviceId, clientId, platform: 'android' }
            }
        });
        const res = result && result.result;
        if (res && res.code === 0) {
            __pushClientIdRegistered = true;
            console.log('[v9.4.0] ✓ PUSH clientId 已注册到 tb_profile.devicePushMap');
        } else {
            // 输出完整 result 便于排查（包括 code / message / _id 等）
            console.warn('[v9.4.0] registerPushClientId 失败 code=' + (res && res.code) + ' message=' + (res && res.message) + ' fullResult=' + JSON.stringify(res));
        }
    } catch (e) {
        console.error('[v9.4.0] registerPushClientId 异常:', e);
    }
}

/**
 * [v9.4.0-fix] 把 broker 配置 + openid 写入 Android SharedPreferences
 * 让 :longconn 进程能读到 broker URL，否则长连接永远连不上
 * 幂等：本次会话只写一次（__mqttConfigRefreshed 守护）
 * 失败兜底：5s 后 __renderSyncBar 会再触发一次
 */
async function __refreshMqttConfig() {
    try {
        if (!window.Android) return;
        if (typeof window.Android.saveMqttConfig !== 'function') {
            console.warn('[v9.4.0-fix] __refreshMqttConfig 跳过：Android.saveMqttConfig 不存在');
            return;
        }
        if (typeof window.Android.getMqttBrokerConfig !== 'function') {
            console.warn('[v9.4.0-fix] __refreshMqttConfig 跳过：Android.getMqttBrokerConfig 不存在');
            return;
        }
        ...
        let openid = null;
        try {
            openid = localStorage.getItem('tb_openid') || null;
        } catch (_) {}
        if (!openid && typeof cachedLoginState !== 'undefined' && cachedLoginState && cachedLoginState.user) {
            openid = cachedLoginState.user.uid || cachedLoginState.user.openid || cachedLoginState.user.id || null;
        }
        if (!openid && typeof DAL !== 'undefined' && DAL && typeof DAL.getCurrentUid === 'function') {
            try { openid = await DAL.getCurrentUid(); } catch (_) {}
        }
        if (!openid) {
            console.log('[v9.4.0-fix] __refreshMqttConfig 跳过：未拿到 openid');
            return;
        }
        ...
        let cfg = {};
        try {
            cfg = JSON.parse(window.Android.getMqttBrokerConfig() || '{}');
        } catch (e) {
            console.warn('[v9.4.0-fix] 解析 broker config 失败:', e);
        }
        const brokerUrl = cfg.brokerUrl || 'wss://broker.emqx.io:8084/mqtt';
        const username = cfg.username || '';
        const topicPrefix = cfg.topicPrefix || 'tb_user_';
        // wss://broker.emqx.io 是公开 broker，不需鉴权
        const password = '';
        const jwt = '';

        // 3. 写入 prefs（saveMqttConfig 内部会启动 LongConnectionService）
        window.Android.saveMqttConfig(brokerUrl, username, password, jwt, openid, topicPrefix);
        __mqttConfigRefreshed = true;
        console.log('[v9.4.0-fix] ✓ MQTT 配置已写入 tb_longconn: broker=' + brokerUrl + ' openid=' + openid.slice(0, 8) + '... topicPrefix=' + topicPrefix);
    } catch (e) {
        console.error('[v9.4.0-fix] __refreshMqttConfig 异常:', e);
    }
}
...
// ============================================================================
// [v9.5.0] Watch 重连机制架构重构 —— 4 层解耦 + 策略模式
// ----------------------------------------------------------------------------
// 重构背景：原 scheduleWatchReconnect 110+ 行单一函数混合 5 个职责（重连调度/状态管理/UI更新/错误处理/业务执行），
// 违反单一职责原则，状态分散在全局变量中难以维护。
//
// 新架构（4 层）：
//   ┌─────────────────────────────────────────────────────────────┐
...
// 设计原则：
//   1. 各层职责单一，可独立测试
//   2. 业务逻辑与 UI 完全解耦（事件驱动）
//   3. 状态集中管理，避免全局变量散落
//   4. 策略可替换（指数退避 / 固定间隔 / 弱网激进模式）
//   5. 兼容旧 API：window.scheduleWatchReconnect(reason) 仍然可用
// ============================================================================
...
(function (global) {
    ...
    // ==========================================================================
    // 1. EventBus —— 极简发布-订阅事件总线
    // ==========================================================================
    class WatchEventBusClass {
        constructor() {
    ...
    // ==========================================================================
    // 2. ReconnectStrategy —— 策略模式：可替换的重连算法
    // ==========================================================================
    class ReconnectStrategy {
        /**
         * 计算下次重连延迟（毫秒）
         * @param {number} attempts 已重试次数
         * @param {Object} context  { maxAttempts, minGapMs, lastAttemptAt }
         * @returns {number} 延迟（毫秒）
         */
        calculateDelay(attempts, context) {
            throw new Error('ReconnectStrategy 子类必须实现 calculateDelay');
        }
         ...
         * @returns {boolean}
         */
        shouldContinue(attempts, context) {
            return attempts < (context.maxAttempts || 8);
        }
    }

    /**
     * 指数退避策略：3s → 4.5s → 6.75s ... 上限 60s
     * 防抖：2s 内只允许一次重连请求（避免 QPS 爆发）
     */
    class ExponentialBackoffStrategy extends ReconnectStrategy {
        constructor(options = {}) {
            super();
        ...
        calculateDelay(attempts, context) {
            const cappedAttempts = Math.min(attempts, this.maxAttempts);
            const backoffDelay = Math.min(
                this.baseDelay * Math.pow(this.multiplier, cappedAttempts),
                this.maxDelay
            );
            // 最小间隔：避免重连过密
            const minGapDelay = Math.max(0, (context.minGapMs || 0));
            return Math.max(backoffDelay, minGapDelay);
        }
    ...
    // ==========================================================================
    // 3. WatchStateManager —— 状态管理（状态机 + 计数器 + 持久化）
    // ==========================================================================
    const WATCH_STATE_KEY = 'tb_watchDegradeState';
    ...
    class WatchStateManagerClass {
        constructor(eventBus) {
            ...
            this.state = {
                ...
                failCount: 0,           // 当前累计失败次数
                firstFailAt: 0,         // 首次失败时间戳
                lastReason: '',         // 最后失败原因
                reconnectAttempts: {    // 5 个 watcher 各自的重试次数
                    profile: 0,
                    task: 0,
                    transaction: 0,
                    running: 0,
        ...
        // ---------- 状态查询 ----------
        getState() {
            return { ...this.state, reconnectAttempts: { ...this.state.reconnectAttempts } };
        }

        getReconnectContext() {
            const maxAttempts = Math.max(0, ...Object.values(this.state.reconnectAttempts));
            return {
        ...
        // ---------- 状态转换 ----------
        markFailure(reason) {
            if (this.state.firstFailAt === 0) this.state.firstFailAt = Date.now();
            this.state.failCount++;
            if (reason) this.state.lastReason = reason;
            // 'ok' → 'degraded'（首次失败即进入降级）
        ...
        markSuccess() {
            if (this.state.status === 'ok' && this.state.failCount === 0) return;
            this.state.status = 'ok';
            this.state.failCount = 0;
            this.state.firstFailAt = 0;
            this.state.lastReason = '';
            this.resetReconnectAttempts();
            this.state.lastTransitionAt = Date.now();
            this.persist();
            this.eventBus.emit('state:changed', this.getState());
            this.eventBus.emit('state:recovered', this.getState());
        }

        incrementReconnectAttempts(watcherType) {
            const key = watcherType || 'all';
            if (key === 'all') {
                Object.keys(this.state.reconnectAttempts).forEach(k => {
                    this.state.reconnectAttempts[k] = Math.min(
                        this.state.reconnectAttempts[k] + 1, 8
                    );
                });
            } else if (this.state.reconnectAttempts[key] !== undefined) {
                this.state.reconnectAttempts[key] = Math.min(
                    this.state.reconnectAttempts[key] + 1, 8
                );
            }
            this.persist();
        }
        ...
        resetReconnectAttempts() {
            Object.keys(this.state.reconnectAttempts).forEach(k => {
                this.state.reconnectAttempts[k] = 0;
            });
            this.persist();
        }
        ...
        /**
         * 计数器超限检测（防御性检查）
         * @returns {string[]} 超限的 watcher key 列表
         */
        getExceededKeys() {
            return Object.entries(this.state.reconnectAttempts)
                .filter(([_, count]) => count > 8)
                .map(([key]) => key);
        }

        /**
         * 转换到 paused 状态（达到重试上限时调用）
         */
        transitionToPaused() {
            ...
            this.persist();
            this.eventBus.emit('state:changed', this.getState());
            this.eventBus.emit('state:paused', { ...this.getState(), previousStatus: prev });
            return true;
        }
        ...
        persist() {
            try {
                localStorage.setItem(WATCH_STATE_KEY, JSON.stringify(this.state));
            } catch (e) {
                console.warn('[WatchStateManager] persist failed:', e?.message);
            }
        }
        ...
        load() {
            try {
                ...
                if (loaded && (loaded.status === 'ok' || loaded.status === 'degraded' || loaded.status === 'paused')) {
                    this.state = {
                        status: loaded.status,
                        failCount: Number(loaded.failCount) || 0,
                        firstFailAt: Number(loaded.firstFailAt) || 0,
                        lastReason: String(loaded.lastReason || ''),
                        reconnectAttempts: {
                            profile: Number(loaded.reconnectAttempts?.profile) || 0,
                            task: Number(loaded.reconnectAttempts?.task) || 0,
                            transaction: Number(loaded.reconnectAttempts?.transaction) || 0,
                            running: Number(loaded.reconnectAttempts?.running) || 0,
                            daily: Number(loaded.reconnectAttempts?.daily) || 0
                        },
                        lastTransitionAt: Number(loaded.lastTransitionAt) || 0
                    };
                }
            } catch (e) {
                console.warn('[WatchStateManager] load failed:', e?.message);
            }
        }
    }

    // ==========================================================================
    // 4. WatchReconnectExecutor —— 业务执行（登录检查 / 重建 Watch / 数据同步）
    // ==========================================================================
    class WatchReconnectExecutorClass {
        ...
        async execute() {
            // 1. 登录检查
            if (!this._isLoggedIn()) {
                console.log('[WatchExecutor] 未登录，取消重连');
                this.stateManager.resetReconnectAttempts();
                return { success: false, reason: 'not_logged_in' };
            }

            // 2. 是否需要重连（registered/connected 全部为 true 则跳过）
            if (!this._needsReconnect()) {
                console.log('[WatchExecutor] 所有连接正常，无需重连');
                this.stateManager.resetReconnectAttempts();
                return { success: true, reason: 'already_connected' };
            }

            // 3. 记录断开详情（用于日志）
            const disconnectedDetails = this._getDisconnectedDetails();
            console.log(`🔄 [WatchExecutor] 执行重连... 断开项: ${disconnectedDetails.join(', ')}`);

            // 4. 真正执行重连
            try {
                if (typeof DAL !== 'undefined' && DAL && typeof DAL.subscribeAll === 'function') {
                    await DAL.subscribeAll();
                } else {
                    throw new Error('DAL.subscribeAll 不存在');
                }
                if (typeof reconcileCloudAfterWatch === 'function') {
                    await reconcileCloudAfterWatch('reconnect');
                }
                console.log('✅ [WatchExecutor] 重连成功');
                return { success: true, reason: 'reconnected' };
            } catch (e) {
                console.error('❌ [WatchExecutor] 重连失败:', e?.message || e);
                return { success: false, reason: 'execution_failed', error: e };
            }
        }
        ...
        _needsReconnect() {
            // 优先使用全局 watchRegistered / watchConnected（如有），否则从 state 推断
            const registered = (typeof watchRegistered !== 'undefined') ? watchRegistered : null;
            const connected = (typeof watchConnected !== 'undefined') ? watchConnected : null;
            if (registered && connected) {
                return Object.values(registered).some(v => !v) ||
                       Object.values(connected).some(v => !v);
            }
            // 降级：基于状态判断
            return this.stateManager.getState().status !== 'ok';
        }

        _getDisconnectedDetails() {
            const details = [];
            const registered = (typeof watchRegistered !== 'undefined') ? watchRegistered : {};
            const connected = (typeof watchConnected !== 'undefined') ? watchConnected : {};
            Object.entries(registered).forEach(([key, reg]) => {
                if (!reg) details.push(`${key}(未注册)`);
            });
            Object.entries(connected).forEach(([key, conn]) => {
                if (!conn && registered[key]) details.push(`${key}(未激活)`);
            });
            return details;
        }
    }
    ...
    // ==========================================================================
    // 5. WatchReconnectScheduler —— 调度器（防抖 + 定时器 + 重试决策）
    // ==========================================================================
    class WatchReconnectSchedulerClass {
        constructor(strategy, stateManager, executor, eventBus) {
            this.strategy = strategy;
            this.stateManager = stateManager;
            this.executor = executor;
            this.eventBus = eventBus;
            this.timers = { pending: null, lastAttempt: 0 };
            this._lastWatchReconnectAt = 0;
            this.WATCH_RECONNECT_MIN_INTERVAL = 5000; // 两次重连之间最小间隔

            // 监听：状态恢复时清理待执行的定时器
            this.eventBus.on('state:recovered', () => this._clearPending());
        }

        /**
         * 调度一次重连（v9.5.0 入口：替代原 scheduleWatchReconnect）
         * @param {string} reason 重连原因
         */
        schedule(reason = 'error') {
            // 1. 全局防抖：debounceMs 内只允许一次
            const now = Date.now();
            if (this.timers.lastAttempt && (now - this.timers.lastAttempt) < this.strategy.debounceMs) {
                console.log(`[WatchScheduler] 全局防抖生效，忽略重连请求 (${reason})，距上次${now - this.timers.lastAttempt}ms`);
                return;
            }
            this.timers.lastAttempt = now;

            // 2. 防止重复调度
            if (this.timers.pending) {
                console.log(`[WatchScheduler] 已有重连任务 pending，忽略本次请求 (${reason})`);
                return;
            }

            // 3. 防御性检查：计数器超限则强制重置
            const exceededKeys = this.stateManager.getExceededKeys();
            if (exceededKeys.length > 0) {
                console.warn(`⚠️ [WatchScheduler] 计数器超限 (${exceededKeys.join(',')})，强制重置`);
                this.stateManager.resetReconnectAttempts();
                if (typeof showToast === 'function') {
            ...
            // 4. 计算延迟
            const ctx = this.stateManager.getReconnectContext();
            const reconnectGap = now - this._lastWatchReconnectAt;
            const minGapMs = Math.max(0, this.WATCH_RECONNECT_MIN_INTERVAL - reconnectGap);
            const delay = this.strategy.calculateDelay(ctx.attempts, { minGapMs, ...ctx });

            // 5. 记录下次重连时间戳（UI 显示倒计时）
            ...
            console.log(`🔄 [WatchScheduler] 计划重连 (原因: ${reason}, 尝试次数: ${ctx.attempts}/${ctx.maxAttempts}, 延迟: ${Math.round(delay/1000)}秒)`);

            // 6. 设置定时器
            this.timers.pending = setTimeout(() => this._execute(reason), delay);
        }

        async _execute(reason) {
            this.timers.pending = null;
            this.eventBus.emit('reconnect:executing', { reason });

            try {
                const result = await this.executor.execute();

                if (result.success) {
                    this._lastWatchReconnectAt = Date.now();
                    this.stateManager.resetReconnectAttempts();
                    this.stateManager.markSuccess();
                    console.log('✅ [WatchScheduler] 重连成功，计数器已重置');
                    this.eventBus.emit('reconnect:success', { reason, result });
                } else {
                    this._handleFailure(reason, result);
                }
            } catch (e) {
                console.error('❌ [WatchScheduler] 重连执行异常:', e?.message || e);
                this._handleFailure(reason, { success: false, reason: 'exception', error: e });
            }
        }

        _handleFailure(reason, result) {
            this.stateManager.incrementReconnectAttempts();
            const ctx = this.stateManager.getReconnectContext();

            // 是否继续重试？
            if (this.strategy.shouldContinue(ctx.attempts, ctx)) {
                console.warn(`⚠️ [WatchScheduler] 重连失败 (${result.reason})，当前重试次数: ${ctx.attempts}/${ctx.maxAttempts}`);
                this.eventBus.emit('reconnect:retry', { reason, attempts: ctx.attempts });
                this.schedule('retry');
            } else {
                // 达到上限 → 转换到 paused 状态 + 触发降级警告
                console.error(`❌ [WatchScheduler] 自动重连已停止（连续 ${ctx.maxAttempts} 次失败）`);
                this.stateManager.transitionToPaused();
                this.eventBus.emit('reconnect:paused', { reason, result });
                // 持久化失败状态（确保重启后仍能感知到降级）
                if (typeof __recordWatchDegrade === 'function') {
                    __recordWatchDegrade();
                }
            }
        }
        ...
        /**
         * 强制立即重连（手动触发 / 用户点击"重试"按钮）
         */
        async forceReconnect(reason = 'manual') {
            this._clearPending();
            this.timers.lastAttempt = Date.now(); // 绕开防抖
            this.eventBus.emit('reconnect:manual', { reason });
            await this._execute(reason);
        }

        _clearPending() {
            if (this.timers.pending) {
                clearTimeout(this.timers.pending);
                this.timers.pending = null;
            }
        }
    }
    ...
    // 初始化：创建单例并暴露到全局
    // ==========================================================================
    const __wbEventBus = new WatchEventBusClass();
    const __wbStateManager = new WatchStateManagerClass(__wbEventBus);
    const __wbStrategy = new ExponentialBackoffStrategy({
        baseDelay: 3000,
        multiplier: 1.5,
        maxDelay: 60000,
        maxAttempts: 8,
    ...
    const __wbExecutor = new WatchReconnectExecutorClass(__wbEventBus, __wbStateManager);
    const __wbScheduler = new WatchReconnectSchedulerClass(__wbStrategy, __wbStateManager, __wbExecutor, __wbEventBus);

    // 暴露到全局
    global.WatchEventBus = WatchEventBusClass;
    global.ReconnectStrategy = ReconnectStrategy;
    global.ExponentialBackoffStrategy = ExponentialBackoffStrategy;
    global.WatchStateManager = WatchStateManagerClass;
    global.WatchReconnectExecutor = WatchReconnectExecutorClass;
    global.WatchReconnectScheduler = WatchReconnectSchedulerClass;
    global.__watchEventBus = __wbEventBus;
    global.__watchStateManager = __wbStateManager;
    global.__watchReconnectStrategy = __wbStrategy;
    global.__watchReconnectExecutor = __wbExecutor;
    global.__watchReconnectScheduler = __wbScheduler;

    // 兼容旧 API：新 scheduleWatchReconnect 入口
    global.__scheduleWatchReconnectNew = function (reason) {
        return __wbScheduler.schedule(reason);
    };

    // 兼容旧 __markWatchFailure / __markWatchSuccess（业务层 5 处 onError 调用）
    global.__markWatchFailure = function (reason) {
        __wbStateManager.markFailure(reason);
        // 触发旧 UI 更新（保持兼容）
        if (typeof updateCloudStatusUI === 'function') updateCloudStatusUI();
    };

    global.__markWatchSuccess = function () {
        __wbStateManager.markSuccess();
        // 清理旧定时器（保持兼容）
        if (typeof watchReconnectTimers !== 'undefined' && watchReconnectTimers.pending) {
            clearTimeout(watchReconnectTimers.pending);
            watchReconnectTimers.pending = null;
    ...
    // 兼容旧 __recordWatchDegrade / __loadWatchDegradeState
    global.__recordWatchDegrade = function () {
        __wbStateManager.persist();
    };
    global.__loadWatchDegradeState = function () {
        __wbStateManager.load();
    };

    console.log('✅ [v9.5.0] Watch 重连机制已重构为 4 层架构（EventBus + Strategy + StateManager + Executor + Scheduler）');
})(window);
