...
function computeHabitBaseRewardFromStreak(task) {
    console.warn('[v9.23.0] [computeHabitBaseRewardFromStreak] 禁止调用此函数！task=', task?.name);
    return 0;
}
...
function updateRunningTimers() { 
    ...
    // [v4.3.8] 已移除末尾多余的 saveLocalCache()
} 
...
// [v9.23.0] 移除经典版 updateClassicBalanceCard（已删除 #balanceCard HTML）
...
// [v9.30.0-perf] 升级为全量序列缓存：仅扫描一次交易（O(N)），各周期从尾部截取，
// 宽屏 2×2 四周期同屏渲染时避免 4×O(N) 重复扫描
let _balanceSeriesCache = { key: null, series: null };
function getFullBalanceSeries() {
    // [v9.30.0] 缓存键含 dataVersion + currentBalance：利息等非交易路径也会改余额，双重保险防陈旧
    const cacheVersion = (typeof dataVersion !== 'undefined') ? dataVersion : -1;
    const cacheKey = cacheVersion + '|' + currentBalance;
    if (_balanceSeriesCache.key === cacheKey && _balanceSeriesCache.series) {
        return _balanceSeriesCache.series;
    }
    ...
    _balanceSeriesCache = { key: cacheKey, series: data };
    return data;
}

function getBalanceDataForPeriod(days) {
    const full = getFullBalanceSeries();
...
// [v9.30.0] compact=true：宽屏 2×2 小图模式，只保留净变动摘要（当前余额在外层统一展示）
function buildBalanceTrendChart(period, compact = false) {
    const allData = getBalanceDataForPeriod(period);
    // [v9.30.0] 宽屏 2×2 四图同屏时 SVG 渐变 id 必须唯一，否则 url(#btArea1) 会跨图串色
    const idSuffix = compact ? ('-p' + (period === 0 ? 'all' : period)) : '';
    ...
    if (isLong) {
        ...
        svg += '<defs>';
        svg += `<linearGradient id="btAreaAll${idSuffix}" x1="0" y1="0" x2="0" y2="1">`;
        ...
        svg += `<path d="${pathD} L ${curvePoints[curvePoints.length-1].x} ${bottomY} L ${curvePoints[0].x} ${bottomY} Z" fill="url(#btAreaAll${idSuffix})"/>`;
        ...
        for (let i = 1; i < curvePoints.length; i++) {
            ...
            svg += `<linearGradient id="btArea${i}${idSuffix}" x1="0" y1="0" x2="0" y2="1">`;
        ...
        for (let i = 1; i < curvePoints.length; i++) {
            ...
            svg += `<path d="M ${prev.x} ${prev.y} C ${cx1} ${cy1}, ${cx2} ${cy2}, ${curr.x} ${curr.y} L ${curr.x} ${bottomY} L ${prev.x} ${bottomY} Z" fill="url(#btArea${i}${idSuffix})"/>`;
        }
        for (let i = 1; i < curvePoints.length; i++) {
            const prev = curvePoints[i - 1], curr = curvePoints[i];
            const segFlow = getFlowColor(allData[i].balance - allData[i - 1].balance);
    ...
    // [v9.30.0] compact 模式（宽屏 2×2）：仅展示净变动，避免四格重复显示当前余额
    const netSign = net >= 0 ? '+' : '';
    const netColor = getFlowColor(net);
    const periodLabel = period === 0 ? '全部' : `近${period}日`;
    let html = '';
    if (compact) {
        html += `<div class="balance-trend-summary balance-trend-summary-compact">`;
    ...
    html = `<div class="balance-trend-summary">`;
...
// [v9.29.1] 报告页近期余额卡片渲染
// [v9.30.0] 宽屏模式：四周期（近7日/30日/90日/全部）2×2 同屏展示，无需切换；
// 当前余额在顶部统一展示一次，每格仅保留净变动
function updateBalanceTrendCard() {
    ...
    const stockColor = getStockColor(currentBalance);
    const periods = [
        { days: 7, label: '近7日' },
        { days: 30, label: '近30日' },
        { days: 90, label: '近90日' },
    ...
    html += `<div class="balance-trend-current"><span class="balance-trend-value" style="color:${stockColor}">${formatHoursShort(currentBalance)}</span><span class="balance-trend-label">当前余额</span></div>`;
    html += `</div>`;
    html += `<div class="balance-trend-grid-2x2">`;
    periods.forEach(p => {
        html += `<div class="balance-trend-cell">`;
        html += `<div class="balance-trend-cell-title">${p.label}</div>`;
        html += buildBalanceTrendChart(p.days, true);
        html += `</div>`;
    });
    html += `</div>`;
...
function updateBalance() {
    // [v9.23.0] 只剩新版（金融版）卡片：直接更新，无需双方案切换
...
async function saveTask(event) {
    ...
    saveLocalCache(); updateAllUI(); hideTaskModal();
...
async function completeTask(taskId) {
    ...
    if (task.isHabit) {
        ...
        // [v9.23.0] NaN 兜底：fixedTime 缺失或非数字时降级为 0，避免 baseReward 传入 NaN
        const safeFixedTime = (typeof task.fixedTime === 'number' && !isNaN(task.fixedTime)) ? task.fixedTime : 0;
        if (safeFixedTime !== task.fixedTime) {
            console.warn(`[v9.23.0] [completeTask] 习惯任务 fixedTime 异常 (${task.fixedTime}) → 0，task=${task.name}`);
        }
...
async function processHabitCompletion(task, baseReward, referenceDate, descriptionDetails = '', pauseHistory = []) {
    ...
    // 3. [v9.0.7] 一次 rebuildHabitStreak 调用，同时获得 prev/new streak
    // 之前两次读：先读 task.habitDetails.streak = oldStreak，再调 rebuildHabitStreak
    // 两次读之间可能因 onRollback/异步操作漂移，导致 oldStreak != prevStreak
    // 现在：rebuildHabitStreak 返回 {prevStreak, newStreak}，单一数据源，杜绝漂移
    ...
    if (shouldAwardBonus) {
        ...
        if (habitBonusReward > 0) {
            ...
            transaction.balanceAdjust = {
                multiplier,
                originalAmount: baseReward + habitBonusReward,
                baseReward: baseReward,
                habitBonus: habitBonusReward
            };
            // 标记为需要重新同步到云端
            transaction._needsCloudUpdate = true;

            // 立即将合并后的完整交易同步到云端
            DAL.updateTransaction(transaction).catch(err => {
...
async function stopTask(taskId) {
    ...
    // [v9.30.0] 结束任务动画（运行中卡必为标准卡）：中性收敛脉冲 + 「⏹ 结束 · 实测时长 · ±入账」徽章。
    // 必须在 updateAllUI() 之前调用（此时运行卡 DOM 仍在原位）；入账/扣费金额取本次新交易（transactions[0] 为最新）。
    if (typeof _celebrateTaskStop === 'function') {
        let animAmount = 0;
        const lastTx = (typeof transactions !== 'undefined' && transactions.length) ? transactions[0] : null;
        if (lastTx && lastTx.taskId === taskId && !lastTx.undone) {
            animAmount = (lastTx.type === 'earn') ? (lastTx.amount || 0) : -(lastTx.amount || 0);
        }
        _celebrateTaskStop(taskId, totalSeconds, animAmount);
    }
...
// [v7.39.2] 统一连胜奖励检测函数：补录和正常记录都使用此函数
// 逻辑：添加交易 → rebuildHabitStreak → 比较 oldStreak vs newStreak → 若增加则发放奖励
// [v7.39.x-revert] 合并习惯奖励到基础交易，保持单条交易
// 找到最近添加的对应任务的交易，在内存中合并奖励信息
function addHabitRewardToTransaction(tx, habitBonusReward, newStreak, multiplier, referenceDate, isBackdate) {
    ...
    txInList.balanceAdjust = {
        multiplier,
        originalAmount: (txInList.balanceAdjust?.originalAmount ?? txInList.amount - bonusAdjusted) + habitBonusReward,
        baseReward: txInList.balanceAdjust?.originalAmount ?? (txInList.amount - bonusAdjusted),
        habitBonus: habitBonusReward
    };

    // 立即同步到云端
    DAL.updateTransaction(txInList).catch(err => {
        console.error('[addHabitRewardToTransaction] ❌ 云端更新失败:', err.code, err.message);
    });
}

// [v7.39.3] prevStreak: the streak value BEFORE the transaction that triggered this check was added.
// This replaces the old approach of reading task.habitDetails.streak inside the function,
// which was already the post-rebuild value (making newStreak > oldStreak always false).
function triggerHabitRewardCheck(task, referenceDate, isBackdate = false, prevStreak, justAddedTxId) {
    if (!task || !task.isHabit) return;

    // 1. oldStreak is passed in from caller (caller captured it before addTransaction)
    const oldStreak = (prevStreak !== undefined && prevStreak !== null) ? prevStreak : (task.habitDetails?.streak || 0);

    // 2. [v7.39.3] No longer call rebuildHabitStreak here — it was already called by the caller
    // (saveBackdate calls rebuildHabitStreak BEFORE calling this function)
    // rebuildHabitStreak(task);

    // 3. Get the current streak (already updated by caller's rebuild)
    ...
    if (newStreak > oldStreak) {
        ...
        if (habitBonusReward > 0) {
            ...
            // 找到刚添加的基础交易（通过 justAddedTxId 或最近一笔同 taskId 的交易）
            let baseTx = justAddedTxId
                ? transactions.find(t => t.id === justAddedTxId)
                : transactions.find(t => t.taskId === task.id && t.type === 'earn');

            if (baseTx) {
                addHabitRewardToTransaction(baseTx, habitBonusReward, newStreak, multiplier, referenceDate, isBackdate);

                // 5. 仅当天显示通知
                const todayStr = getLocalDateString(new Date());
                const refDateStr = referenceDate instanceof Date
...
// [v7.39.2] 辅助函数：计算指定 streak 对应的习惯奖励
function calculateHabitReward(task, streak) {
    if (!task.habitDetails?.rewards || task.habitDetails.rewards.length === 0) return 0;
    let total = 0;
    task.habitDetails.rewards.forEach(rule => {
        if (streak >= rule.start) {
            let reward = rule.type === 'fixed' ? rule.value : rule.value * streak;
            if (rule.limit && reward > rule.limit) reward = rule.limit;
            total += reward;
        }
    });
    return total;
...
async function saveBackdate(event) {
    ...
    for (let i = 0; i < completionCount; i++) {
        ...
        if (task.type === 'reward') {
            ...
            if (task.isHabit) {
                ...
                description += ' (补录)';
                // [v7.39.3] FIX: capture prevStreak BEFORE addTransaction, then call AFTER rebuildHabitStreak
                // (previous position was here, BEFORE tx was added — making oldStreak always equal newStreak)
                const prevStreak = task.habitDetails?.streak || 0;
                // addTransaction called below at line 5817
                // [v7.39.3] triggerHabitRewardCheck moved to AFTER rebuildHabitStreak (see after the addTransaction block)
                window._pendingHabitPrevStreak = prevStreak;
            }
        ...
        // Read prevStreak from window flag (set by reward type block below), then clear it
        if (transactionType === 'earn' && task.isHabit && ['continuous', 'continuous_target'].includes(task.type)) {
            const prevStreak = (window._pendingHabitPrevStreak !== undefined) ? window._pendingHabitPrevStreak : (task.habitDetails?.streak || 0);
            window._pendingHabitPrevStreak = undefined;
            rebuildHabitStreak(task);
            triggerHabitRewardCheck(task, backdateTimestamp, isBackdate, prevStreak, baseTxId);
        }

        // [v7.39.3] reward 类型习惯：读取 prevStreak 并触发连胜奖励检测
        // (prevStreak was captured before addTransaction at line 5717, stored in window flag)
        if (transactionType === 'earn' && task.isHabit && task.type === 'reward' && window._pendingHabitPrevStreak !== undefined) {
            const prevStreak = window._pendingHabitPrevStreak;
            window._pendingHabitPrevStreak = undefined;
            rebuildHabitStreak(task);
            triggerHabitRewardCheck(task, backdateTimestamp, isBackdate, prevStreak, baseTxId);
        }
    ...
    if (didHabitBackdate) {
        ...
        // [v7.39.3] Note: per-iteration rebuildHabitStreak already ran for each habit tx above.
        // This final call handles the case where no per-iteration rebuild ran (non-habit txs in the loop).
        rebuildHabitStreak(task);
        // [v7.39.3] Clean up any stale window flag
        window._pendingHabitPrevStreak = undefined;
...
// [v9.0.7] 习惯系统重构 - rebuildHabitStreak 单一数据源
// 核心改动：
//   1) 永远从 transactions 数组读取，不依赖 transactionIndex
//   2) 返回 {prevStreak, newStreak, lastCompletionDate} 给 processHabitCompletion
...
function rebuildHabitStreak(task) {
    ...
    // addToTransactionIndex 添加 1 笔新交易后，索引中只含 1 笔
    // 导致 streak 算成 1，覆盖原本的连胜数据
    const prevStreak = task.habitDetails?.streak || 0;
    const prevLastCompletionDate = task.habitDetails?.lastCompletionDate || null;

    console.log(`[rebuildHabitStreak] 🔄 重建连胜: ${task.name} (prevStreak=${prevStreak})`);

    // 1. [v9.0.7] 从 transactions 数组读取，不再使用 transactionIndex
    const taskTransactions = transactions
    ...
    for (const tx of taskTransactions) {
        ...
        // [v9.0.7] 使用本地时区（Asia/Shanghai）切片周期起始日
        // 之前用 txDate.getFullYear/getMonth/getDate 是浏览器本地时区
        // 在用户时区 ≠ 系统时区时可能出现边界错位
        const localDate = new Date(txDate.getTime() + (new Date().getTimezoneOffset() + 8 * 60) * 60 * 1000);
        // 上面的转换等价于 +8 小时，但保持代码兼容 getLocalDateString
        let periodStartDate;
        if (period === 'daily') {
    ...
    // [v9.0.7] 返回 prev/new 给 processHabitCompletion，避免两次读数据漂移
    return {
        prevStreak,
        newStreak,
