...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.34.2`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**分阶段冷启动 + 同步链路 P0 优化**——冷启动首屏不再等全量交易（首页 11s→约 1.5s，剩余页后台续载 + 结算/连胜门控保护，延续 9.22.S 安全纪律）；Watch 回推去重+防抖（落地「待优化：Watch 监听回推性能问题」章节）；activeSync 智能跳过（5 分钟兜底）；getDelta 复合游标（同毫秒漏数隐患根治）；txId O(1) 索引。

> ✅ v9.34.1 已推送（2026-08-15）。v9.34.2 开发中。
