...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.32.1`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**首页三卡片切换动画 + 报告页宽屏双列布局 + 近期余额图表修复**——首页余额/屏幕时间/睡眠三卡片纳入页面切换动画（堆叠/展开整体弹入）；报告页宽屏模式重构为最简单的双列布局（卡片两两组合等高）；近期余额图表修复右端标注截断、90日/全部最小间距1天。

> 🔄 v9.32.1 开发中（首页三卡片切换动画 · 报告宽屏双列 · 近期余额图表修复）。下一版本目标待定。
