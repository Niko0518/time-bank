...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.30.1`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**设计感与动效升级（第一阶段）**——解决应用“缺乏设计性、层次感，观感廉价”的问题，对标 Telegram 的丝滑弹性动画与 iOS 的排版设计感。

> ✅ v9.30.1 已推送完成（双卡堆叠 · 平板屏幕时间修复：未启用屏幕时间管理时睡眠卡片也形成堆叠效果、平板端屏幕时间卡片/小组件显示为 0 的跨设备行为差异修复）。下一版本目标待定。
