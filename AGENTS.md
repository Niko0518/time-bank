...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.34.1`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**习惯连胜修复 + 手动补录配额对齐 + 详情行公式化**——习惯连胜改按周期序号判断连续（每周不必同一天完成也能累计）；手动补录消费任务与正常消费一致（额度内 50%、超出 200%、动态倍率）；任务详情改为多层嵌套公式（圆括号基础组、方括号总和组、外层倍率着色）。

> ✅ v9.34.1 已推送（2026-08-15）。下一版本目标待定。
