...
## 2.5 ⚙️ 工作流自动化脚本（v9.17.11 起改为参考，不再强制）

> 🆕 **v9.17.11 教训**：`scripts/sync-all.ps1`、`scripts/bump-version.ps1` 等自动化脚本曾导致 `app-1.js` 被意外清空并推送到远程仓库。经排查，脚本化流程隐藏了文件状态，AI 无法直观确认每一步的实际结果，风险高于收益。
>
> 📌 **自 v9.17.11 起，所有"流程性工作"改为手动执行**，AI 必须在每一步用 `Get-Content` / `Read` / `git diff --stat` / `git status --short` 等工具确认实际结果，禁止盲信脚本输出。
>
> 📌 脚本仍保留在 `scripts/` 目录，仅作为**参考实现**，可查阅其命令逻辑，但不再默认调用。

### ✅ 推荐手动流程（替代原脚本）

| 步骤 | 原脚本 | 替代手动命令/操作 |
|------|--------|------------------|
| 版本号同步 | `bump-version.ps1` | 使用 Edit/Replace 或一次性 Python 脚本精确修改 7 处，修改后立即 `Select-String` / `Grep` 验证 |
| 双端同步 | `sync-www.ps1` / `sync-all.ps1` | 使用 3.1 节的手工 `Copy-Item` 命令，复制后用 `Get-FileHash` 或 `(Get-Item).Length` 验证 |
| 推送 | `sync-all.ps1 -Commit -Push` | `git add -A` → `git status --short` → `git commit` → `git push` |
| 安装 | `log&data\sync.ps1` | `.\android_project\gradlew assembleDebug -p android_project` → `adb install -r -g "android_project\app\build\outputs\apk\debug\app-debug.apk"` → `adb shell am start -n com.jianglicheng.timebank/.MainActivity` |

### ❌ 禁止调用的脚本清单（v9.17.11+）

以下脚本仍保留，但**禁止在常规工作流中调用**，仅用于查阅其实现逻辑：
...
- `scripts/check-js-syntax.ps1`
- `scripts/pre-push-v9.15.0.ps1`

> 例外：开发者**明确**要求使用某个脚本时，可单次调用，但调用后必须人工验证结果。
...
> 🆕 **v9.17.11+ 手动流程**：上述"双端同步"步骤**禁止使用** `scripts/sync-www.ps1` / `scripts/sync-all.ps1`，必须使用本节的手工 `Copy-Item` 命令，复制后立即用 `Get-FileHash` 或 `(Get-Item).Length` 逐项核对。
> 版本号同步同样改为手动修改，修改后执行 `Select-String` / `Grep` 自检，确认 7 处一致。
