...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.31.0`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**分类任务迷你卡扩展**——将迷你卡片能力从「最近任务」扩展到「分类任务」，设置页开关升级为四档范围滑块（关闭/最近/分类/全部），「全部任务」标题更名为「分类任务」。

> ✅ v9.31.0 已推送完成（分类任务迷你卡 · 范围滑块 · 全部任务改名：迷你卡片范围四档滑块、升格状态按 scope 隔离、分类任务迷你卡复刻最近任务全部规则、设置页滑块重构为 .style-switcher 风格）。下一版本目标待定。
