...
3. 列出将修改的文件清单
4. 说明风险/副作用（如有）
5. 🆕 **必读**：[`scripts/` 工作流自动化脚本清单（章节 2.5）](#25-工作流自动化脚本v9178-强制规范) —— 涉及版本号、双端同步、自检时**强制**调用脚本，不允许手动 SearchReplace / Grep
...
## 2.5 ⚙️ 工作流自动化脚本（v9.17.8+ 强制规范）

> 🆕 **自 v9.17.8 起，每次发版的"流程性工作"必须使用 `scripts/` 下脚本，不再使用手动 SearchReplace / Grep。**
>
> 📌 **本项目的开发者是 AI，不是人类**。AI 的工作流成本 = 工具调用次数 + 易错环节排查。所以本项目"流程性优化"的核心目标是：
> 1. **减少 AI 工具调用次数**（每次 Read/SearchReplace/Grep 都是 token 开销）
> 2. **消除易错环节**（BOM 字符、序号错位、漏改）
> 3. **缩短决策路径**（不用每次想"现在该改哪几处"）

### ❌ 已明令禁止的优化方向

| 方向 | 为什么禁止 |
|------|-----------|
| **技术日志模板化 / 用户日志章节填空模板** | **僵化**。日志是技术决策的留痕，章节结构不能定死。本项目开发者是 AI，AI 写 markdown 本就是秒级，"节省撰文时间"这个动机不成立。 |

> AI 助手收到"优化日志"的提议时，默认**拒绝**——除非开发者明确要求临时模板用于特殊场景（如批量脚本重构）。

### ✅ 必须使用的脚本清单

| 脚本 | 路径 | 职责 | 何时使用 |
|------|------|------|---------|
| **verify-version.ps1** | `scripts/verify-version.ps1` | 检查 7 处版本号是否同步 + 检测残留 | 每次改完版本号后**自动**（其他脚本内部已调用） |
| **bump-version.ps1** ✅ | `scripts/bump-version.ps1` | 一键同步 7 处版本号 + 自动自检 | 收到"递增版本号 X.Y.Z"指令后**强制**使用，禁止手动 SearchReplace |
| **sync-www.ps1** ✅ | `scripts/sync-www.ps1` | `assets/www/` 同步根目录 + 启本地 HTTP 预览 | 跨设备调试 / 不依赖 USB 的浏览器验证场景 |
| **sync-all.ps1** ✅ | `scripts/sync-all.ps1` | 推送前 Android → 根目录 + hash + 版本号自检 + (可选) git commit/push | 收到"推送"指令后**强制**使用 |
| **verify-hash.ps1** | `scripts/verify-hash.ps1` | 双源镜像 hash 一致性校验 | 推送后、sync-all 完成后 |
| **check-js-syntax.ps1** | `scripts/check-js-syntax.ps1` | JS 语法快速校验 | 大量 JS 修改后 |
| **pre-push-v9.15.0.ps1** | `scripts/pre-push-v9.15.0.ps1` | 推送前综合自检 | 推送前 |

### 📋 各脚本详细说明

#### scripts/verify-version.ps1（v9.17.8 已固化）

**职责**：检查 7 处版本号（4 个文件）是否一致，输出 PASS/FAIL。

**输出示例**：
```
=== 权威源 (android_project/app/src/main/assets/www) ===
  ✅ index.html:L12   <title>v9.17.8</title>
  ✅ index.html:L243  副标题 v9.17.8
  ✅ index.html:L1341 关于页 v9.17.8
  ✅ app-1.js:L15     APP_VERSION = 'v9.17.8'
  ✅ sw.js:L1,L14     CACHE_NAME + 注释
  ✅ build.gradle:L15-16  versionCode 87 + versionName "9.17.8"

=== Android 工程 (build.gradle) ===
  ✅ versionCode 87
  ✅ versionName "9.17.8"

=== 是否有旧版本残留? ===
  [警告] 发现 v9.17.7 残留（仅允许历史日志条目）

=== 是否有 v9.17.8? ===
  ✅ 全部命中
```

#### scripts/bump-version.ps1 ✅（v9.17.8 立即可用）

**职责**：一行命令改 7 处版本号 + 自动自检（基于正则，不依赖精确字符串匹配 → 规避 BOM 陷阱）。

**用法**：
```powershell
# 完整用法：手动指定版本号 + 副标题
.\scripts\bump-version.ps1 -NewVersion "9.17.9" -VersionCode 88 -Subtitle "新功能特性词组"

# 简化用法：自动递增小版本（v9.17.8 → v9.17.9）
.\scripts\bump-version.ps1 -Subtitle "新功能特性词组" -AutoIncrement
...
**AI 必读**：
- 收到"递增版本号 X.Y.Z"指令后，**必须**调用本脚本
- **禁止**手动用 SearchReplace 改 7 处版本号（v9.17.8 之前曾因 BOM 字符翻车）
- 脚本会自动调用 `verify-version.ps1` 自检，无需手动再跑

#### scripts/sync-www.ps1 ✅（v9.17.8 立即可用）

**职责**：
1. `assets/www/*` → 根目录 `index.html` / `js/` / `css/` / `sw.js` / `manifest.json`
2. 同步后自动跑 hash 校验（11 个文件）
3. 可选：启 `python -m http.server` 输出预览 URL
...
# 同步 + 本地预览（不依赖 USB）
.\scripts\sync-www.ps1 -Preview -Port 8000

# 预览模式（不实际复制）
.\scripts\sync-www.ps1 -DryRun
```

#### scripts/sync-all.ps1 ✅（推送前必用）

**职责**：综合推送前流程——
1. 同步 Android → 根目录（封装 sync-www.ps1）
2. Hash 校验（ALL-HASH-OK 才继续）
3. 7 处版本号自检
4. (可选) `git add -A` + `git commit`
5. (可选，需显式 `-Push`) `git push`
...
# 同步 + 自检 + git commit
.\scripts\sync-all.ps1 -Commit "v9.17.9: 修复xxx"

# 同步 + 自检 + git commit + git push（push 必须显式）
.\scripts\sync-all.ps1 -Commit "v9.17.9: 修复xxx" -Push
```

**AI 必读**：
- 收到"推送"指令后**强制**使用本脚本
- **禁止**手动写 Copy-Item 命令
- **push 默认不执行**——必须显式加 `-Push`
...
> 🆕 **v9.17.8+ 强制**：上述"双端同步"步骤**必须使用** [`scripts/sync-www.ps1`](#scriptssync-wwwps1v9178-立即可用) 或 [`scripts/sync-all.ps1`](#scriptssync-allps1推送前必用)，不允许手动写 Copy-Item 命令。
> 同理，版本号同步**必须使用** [`scripts/bump-version.ps1`](#scriptsbump-versionps1v9178-立即可用)。
