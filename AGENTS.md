...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.34.0`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**Turbo 模式 + 金融利率改革 + 宽屏双列布局**——新增 Turbo 快速赚取模式（所有获取/消费×1.5、利率锁定 0.5%、与均衡模式互斥）；全面取消负余额 1.2 倍惩罚、金融系统利率锁定 1%；报告/设置页宽屏双列布局重构，修复设置页串页问题。

> 🔄 v9.34.0 开发中（Turbo 模式 · 利率改革 · 宽屏双列）。下一版本目标待定。
