...
| 维度 | 内容 |
|------|------|
| **当前版本** | `v9.35.0`（实时更新，详见下方「🎯 当前版本目标与进度」章节） |
...
### 版本主题
**语音指令 + AI 深度建议 + 弹窗重构**——首页悬浮麦克风按钮（接管原创建任务按钮位置），用户对手机说"开始/完成/结束/删除/暂停/继续/取消 某某任务"即可直接操作；本地正则解析高频指令（零 AI 消耗）+ AI 意图解析兜底复杂指令；自录音+云 ASR 方案绕过荣耀设备系统语音引擎限制；AI 对话弹窗重构为轻盈浮动卡片（情感化设计，进入不自动唤起键盘，语音输入直接转文字显示）；AI 通道改用 CloudBase 套餐内 AI 额度（混元 hy3，零边际成本）；首页隐藏 Watch 连接诊断按钮（功能保留）。

> ✅ v9.35.0 已推送（2026-08-18）。下一版本目标待定。
