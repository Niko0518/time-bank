...
## v7.11.2 (2026-02-02) - 屏幕/睡眠设置持久化修复

### 问题背景
卸载重装或重启后，屏幕时间管理与睡眠时间管理被当作新设备，设置项与分类标签无法持久保存。

### 解决方案
1. 屏幕/睡眠设置初始化时按 `lastUpdated` 比较本地与云端配置，避免旧配置覆盖新配置。
2. 当前设备缺少云端配置时，自动从“最近设备配置”恢复并同步到当前设备。

### 关键改动
**文件**: `index.html`

- `getLatestDeviceSettings()`：选择云端最近更新的设备配置
- `initScreenTimeSettings()`：按时间戳选择本地/云端，并在缺失时迁移最近设备配置
- `initSleepSettings()`：按时间戳选择本地/云端，并在缺失时迁移最近设备配置

## v7.11.1 (2026-02-02) - 关联应用编辑与睡眠倒计时修复

### 问题背景
1. 任务编辑中修改“关联应用”后保存，启动仍回到旧关联应用（输入值未生效）
2. 夜间模式进入睡眠不再显示入睡倒计时，直接开始记录

### 解决方案
1. 输入框手动修改时清除旧的 `selectedPackage`，避免保存时被旧包名覆盖
2. `resolveAppPackage()` 仅在输入值与已选应用匹配时才使用 `selectedPackage`
3. 初始化与进入睡眠时规范化 `sleepSettings.countdownSeconds`，确保倒计时始终可用

### 关键改动
**文件**: `index.html`

- `filterAppList()`：手动输入时清除旧包名，避免关联应用保存回退
- `resolveAppPackage()`：校验输入与选中应用一致才使用选中包名
- `initSleepSettings()` / `startSleepMode()`：倒计时配置异常时重置为 60 秒
- 通透模式样式：提升睡眠报告弹窗文字对比度，增强自动检测补录“?”按钮可读性
- 首页卡片间距：展开时余额卡片与屏幕时间卡片的间距统一为与屏幕时间/睡眠卡片一致
- 任务历史弹窗：通透模式下日期筛选文字提升可读性
- 饼图任务视图：屏幕时间“最近记录”按赚取/消费类型分别筛选
- 趋势演变图：Tooltip 底部提示与进度条样式对齐热力图/饼图，长按进度条同步动画
- 说明弹窗文案：时间仪表盘/趋势演变交互说明补充点击进入详情与长按3秒自动进入；屏幕时间/自动检测说明补充“首次启用不追溯、多设备汇总、失败兜底提示”
- 均衡模式：改为云端唯一真相，忽略本地缓存
- 自动检测补录：改为多设备原始记录（`autoDetectRawRecords`）+ 汇总生成最终补录/修正交易
- 自动检测处理日期：云端全局记录（`autoDetectProcessedDates`），避免重复结算
- 屏幕时间/自动检测：首次启用后不追溯历史日期（基于 `enabledDate`）
- 网页端长时间休眠恢复：若期望登录但未恢复 UID，跳过本地缓存并等待云端恢复，防止旧数据覆盖云端
- 网页端恢复超时兜底：强制刷新登录状态并重拉云端数据，仍失败则提示重新登录
- 权限管理：新增设置页权限状态与快捷授权入口（使用情况访问/悬浮窗/通知/精准闹钟/电池优化）
- 权限管理：已授予权限自动归档到“已授予权限”以节省空间
- 设置页精简：移除“数据修复工具”入口
- 趋势演变图：优化滑动提示动画，使用平滑插值与高度锁定减少跳跃闪烁
- 趋势演变图：支持点击打开每日详情，长按 3 秒自动进入详情
- 设置页精简：移除“余额不足提醒”“通知诊断”“认识设置系统”入口
