...
### [Core] 推荐任务 w5：提醒命中从离散开关改为平滑距离权重

#### 背景

v9.20.0 重构了 w1（稳定性 × 集中性）、w2（重要性 × 紧急性）并删除了 w4，但 w5（提醒命中）仍保留早期实现：单点提醒 ±30min 内为 1、区间/跨夜提醒命中为 1，否则为 0。这种 0/1 跳变导致推荐列表在提醒边界处出现抖动，且所有提醒（早晨 8 点与深夜 22 点）权重相同，无法表达"距离提醒时间越近越该推荐"的直觉。
...
- 提醒任务从"到点突然置顶"变为"提前 1-2 小时温和爬升"
- 区间提醒不再在整段时间内保持满权重，越接近区间末端权重越高
- 跨夜提醒（如 22:00-06:00）在白天 14:00 时距两端点均 8h，权重 ≈ 0，避免误推

---

## v9.20.0 (2026-07-14)

### [Core] 推荐任务算法重构：w1/w2/w4 三维调整
...
- `_computeAlgoScore` 函数（[app-1.js#L8661](file:///d:/TimeBank/android_project/app/src/main/assets/www/js/app-1.js#L8661)）
- 新增辅助函数：`_stability`、`_countActiveDays`、`_build48BucketHist`、`_concentration`、`_streakImportance`、`_dailyUrgency`
