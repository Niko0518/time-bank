...
## v9.36.2 (2026-08-20) — Time Bot 体验精修 + 弹窗层级统一

### 核心变更

1. **弹窗 z-index 层级统一（架构）**：建立五层体系——内容(0/1) < 悬浮控件(1000) < 特效(1300) < 交互浮层(1600) < 弹窗(2000)。修复任务结算/粒子等特效（原 1200/1000）偶尔覆盖到弹窗上方的问题；统一弹窗基座为 2000（与 `.modal` 一致），消除各浮层 1001/1002/1500 混乱取值导致的错误覆盖/穿透。
2. **聊天弹窗防频闪/防高度跳变**：`_chatLoadHistory` 新增可重入锁 `_histLoading` + 防重建标记 `_histLoaded`（仅首次真正加载云端历史，复开复用已有内容，不再 clear→重建）；移除静态默认问候、历史为空时动态插欢迎语；CSS 固定高度 `min(58vh,460px)` + 内部滚动，杜绝加载历史后瞬间变高。
3. **FAB 长按后秒点取消**：ai-voice.js `_pttGuard` 由固定 800ms 阻塞改为一次性消费；取消判定移至 `pointerup`（不再依赖时序不可靠的合成 click），松手即可立即点取消。
4. **手动结束任务不弹气泡**：`stopTask(taskId, fromVoice=false)`（语音指令结束传 true）；`TimeBot.onStop(task, seconds, fromVoice)` 仅 `fromVoice=true` 时 say 气泡确认，手动按钮只保留 react 视觉反馈（highlight 判定逻辑不变）。
5. **Time Bot 夜间恒定配色**：`_applyScheme` 固定 `setInk('#161513')` + `setEyeColor('#f6f1e6')`，绕开引擎默认 light-dark()；移除暗色主题对聊天气泡的 CSS 覆盖。

### 收益
- 彻底规避特效/弹窗层级错误覆盖，弹窗交互稳定无频闪；语音/手动结束反馈更贴合直觉，夜间观感统一。

## v9.36.1 (2026-08-20) — 开发流程精简（一键脚本）

### 核心变更

1. **bump-version.ps1 一键版本号升级 / 双端同步 / 校验**：早期流程繁琐（11 处版本号手动改、5 条 Copy-Item、5 重自检），模型性能提升后 token 消耗加大需简化。脚本自动完成：从 `app-1.js` 读当前版本 → 替换全部纯版本号位置（title / 副标题 / 关于页 / APP_VERSION / CACHE_NAME / sw 注释 / versionName / versionCode 自动 +1 / AGENTS.md）→ 5 条 Copy-Item 串行同步（失败即停）→ diff 强校验 → 版本位置残留扫描 + 全库旧版本快照。BOM 文件读写保留 BOM，精确匹配不触碰历史注释 / 历史版本日志。
2. **精简技术日志格式**：单版本技术日志从 30-200 行压缩为 10-30 行要点，降低上下文加载成本。
3. **AGENTS.md 精简**：5 重自检等防御规则改为脚本指引，文档从 ~650 行精简至 ~523 行。

### 收益
- 版本收尾从大量手动工具调用缩减为一条脚本命令，显著节省 token 与时间。
...
- 一阶段从“原地缩小”改为“飞行进入”：迷你卡幽灵从排序位平移到标准卡将出现的位置（取消“返回原位”的逆过程）。仅位移故**保留毛玻璃**（位移 blur 性能安全，观感与真卡一致）
- 二阶段：标准卡放大幽灵（scale 0.5→1，弹性曲线 + opacity 线性淡入双轨道）+ 其他卡 FLIP 让位（540ms / 30ms 错峰 / 无回弹）+ 被裁卡挤出
- 落点预测量：新增 `_measureCardRectInFinalLayout`，用等宽隐藏容器预渲染最终布局，量出标准卡位置供飞行瞄准
- 放大幽灵剥离 blur 后**按位置采样真实背景补底**（新增 `_sampleBackdropColor` 对主题渐变按卡片中心插值 + 叠回玻璃高光层），替代旧固定亮色(#3b4664)避免“泛白卡”
- 三拍节奏体系：移动 400ms / 放大 420ms / 挤走 540ms

### [UX] 退场动画语义区分

- 碎裂（`_shatterDisappearingMiniCards`）：任务完成/删除 → 原地 16 粒子径向飞散消散
- 挤出（`_pushOutDisappearingMiniCards`）：升格被标准卡挤掉 → 整卡向下滑出约 1.3 卡高（仅位移保留毛玻璃），两种退场语义不混用

### [Core] 毛玻璃强度统一控制（分级等比缩放）

- **背景**：早有“通透强度”滑块（`--glass-blur-scale` / `--glass-opacity-scale`），但仅主力卡片接入；大量毛玻璃用硬编码 blur，滑块调了不动 → 通透效果割裂
- **排查**：全文件清点出 7 种硬编码基准值（2/4/8/10/12/20/30px），散布于弹窗/面板/FAB/标签/睡眠条/AI 伙伴卡等
- **方案**：全部 `blur(Npx)` → `blur(calc(Npx * var(--glass-blur-scale)))`（含 -webkit- 与同行合并写法），各元件保留适配自身尺寸的基础模糊量、全部随滑块等比缩放（用户选定“分级等比缩放”）
- **透明度**：主力玻璃卡已接入 `--glass-opacity-scale`；功能性元素（弹窗/提示/FAB）透明度为可读性保留固定值，不宜等比缩放
- 性能优化（滚动降模糊等）按用户要求日后再议

### [Fix] 获得/消费页切换动画毛玻璃全透明（用户“心结”）

- **根因**：切换动画加在容器（`.recent-tasks-grid` / `.category-tasks`）上，容器 opacity/transform 形成“背景根（backdrop root）”，把子卡片毛玻璃的取景区隔离 → 动画期间子卡 backdrop-filter 取不到背景，整卡全透明。报告/设置页动画加在卡片自身（`.report-section`/`.settings-section`）故无此问题
- **修复**：动画目标从容器改为卡片自身（`.recent-tasks-grid .task-card` / `.category-tasks .category-header` + `.task-card`），对齐报告/设置页做法；卡片自动画不击穿自身毛玻璃
- **配套**：移除 `renderTaskCards` 写在卡片上的死值 `--spring-i`（原无 CSS 读取），让分类卡片继承 `.category-tasks` 容器的分类序号，保持“块级错峰”节奏；最近网格卡片同步=整体平移
...
**新增功能**：在每日详情弹窗中引入饼图，展示当日时间分配结构。

**实现细节**：

1. **饼图数据聚合**（app-reports.js `renderDayDetailPie`）：
   - 按任务标题聚合当日交易记录
   - 展示 Top5 任务 + "其它"分类
   - 虚拟预计记录（屏幕时间奖励、利息）：仅在当日未结算时注入饼图展示，不影响汇总行计算

2. **汇总行布局**（app-reports.js + main.css）：
   - 三列横排：获得 / 消费 / 净值
   - 净值置于第三列（最右）
   - 字号自适应（clamp），避免换行
   - ± 符号置于时长前，earn 段省略 + 号
...
3. **系统任务处理**：
   - 睡眠、屏幕时间等系统任务采用 `buildTaskViewColorMap` 的构成分析色系
   - 系统任务的方向通过 ± 符号区分
...
**复用策略**：饼图完全复用构成分析的 `.pie-chart*` / `.pie-legend*` / `.pie-center*` / `.pie-slice-label*` CSS 类，避免样式冗余。
